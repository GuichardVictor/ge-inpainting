...
class PConvModel:
    ...
    def __init__(self, img_shape=(128, 128, 1)):
        ...
        self.model, self.masks = self._create_model()
    ...
    def _create_model(self):
        ...
        def _decoding_layer(img_in, mask_in, enc_img, enc_mask, filters, kernel_size, norm=True):
            ...
            concat_img = Concatenate(axis=3)([enc_img, up_img])
            concat_mask = Concatenate(axis=3)([enc_mask, up_mask])
        ...
        inputs_img = Input(self.shape, name='inputs_img')
        inputs_mask = Input(self.shape, name='inputs_mask')
        ...
        d_conv8, d_mask8 = _decoding_layer(d_conv7, d_mask7, inputs_img, inputs_mask, self.shape[2], 3, norm=False)

        outputs = Conv2D(self.shape[2], 1, activation = 'sigmoid', name='outputs_img')(d_conv8)

        model = Model(inputs=[inputs_img, inputs_mask], outputs=outputs)
        
        return model, inputs_mask # return masks for loss computations
    
    def _compile_model(self, model, masks, lr=0.0002):
        model.compile(optimizer=Adam(learning_rate=lr), loss=self._vgg_custom_loss(masks), metrics=[self.PSNR, self.SSIM])
    
    def _vgg_custom_loss(self, mask):
